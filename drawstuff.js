...
function deepCopy(anInstance) {
  ...
  for (var incKey in incOut)
    ...
    else incOut[incKey] = anInstance[incKey];
...
function interpRect(
  ...
  try {
    if (
      typeof tlAttribs !== "object" ||
      typeof trAttribs !== "object" ||
      typeof brAttribs !== "object" ||
      typeof blAttribs !== "object"
    )
      throw "InterpRect: passed attributes are not objects";
    else {
      let tlSize = Object.keys(tlAttribs).length,
        trSize = Object.keys(trAttribs).length;
      let brSize = Object.keys(brAttribs).length,
        blSize = Object.keys(blAttribs).length;
      if (tlSize !== trSize || tlSize !== brSize || tlSize !== blSize)
        ...
        var laDelta = {},
          raDelta = {}; // left and right attribute deltas
        for (var a in tlAttribs)
          if (typeof tlAttribs[a] == "number") {
            ...
            // assume attrib is an object
            laDelta[a] = blAttribs[a]
              .clone()
              .subtract(tlAttribs[a])
              .scale(vDelta);
            raDelta[a] = brAttribs[a]
              .clone()
              .subtract(trAttribs[a])
              .scale(vDelta);
          } // end if attrib is an object

        // set up the horizontal interpolation
        var ha = {},
          haDelta = {}; // horizontal color and delta
        ...
        for (var y = top; y <= bottom; y++) {
          ...
          for (var a in ha)
            if (typeof ha[a] == "number")
              haDelta[a] = hDelta * (ra[a] - la[a]); // assume attrib is object
            else haDelta[a] = ra[a].clone().subtract(la[a]).scale(hDelta);
          for (var x = left; x <= right; x++) {
            // for each pixel column
            shadePixel(imagedata, x, y, globals, ha);
            for (var a in ha)
              if (typeof ha[a] == "number")
                ha[a] += haDelta[a]; // assume attrib is object
              else ha[a].add(haDelta[a]);
